...
int serial_buffer_ptr = -1; // -1 means buffer is empty
int serial_buffer_len; // How many bytes are in the serial buffer
...
void eptri_usb_send(const void *data, int total_count) {
    eptri_usb_send_epno(data, total_count, 0);
}
...
int eptri_usb_getc(void) {
    ...
    if ((serial_buffer_ptr >= 0) && (serial_buffer_ptr < serial_buffer_len)) {
        c = serial_buffer[serial_buffer_ptr++];
    }
